...
# mmm=8
li=[]
for i in range(mmm):  # 0,1,2,3,4,5,6,7
    li.append(i*(mmm-1)+1) # 1,8,15,22,29,36,43,50
print(li)
...
for m,n in enumerate(li):
    ...
    with open('./'+"result/"+str(n)+'/resultabcd.json',encoding='utf-8')as f: #./100/resultabc.json #./101/resultabc.json
        ...
        for i in range(4*(mmm-1)): # 对于每一行数据
            if i==0:
                # print("初始化")
                dictMerged = dict(temp[i])
                for i in dictMerged.items():
                    if i[0]!='entries':
                        dictMerged[str(i[0])]= [dictMerged[str(i[0])]] #相当于不做任何处理
                # print("dicMerged-1----------------------------------------------------------------------")
                # print(dictMerged)
